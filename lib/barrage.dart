...
class BarrageInit extends StatefulWidget {
  final Map cfg;
  BarrageInit({Key? key, this.cfg = Cfg_info}) : super(key: key);
...
class BarrageInitState extends State<BarrageInit> {
  BarrageWallController _controller = BarrageWallController.all(
    options: ChannelOptions(height: 24.0),
    channelCount: 25,
  );
  BarrageData barrageDatas = BarrageData();
  var _timer = Timer(Duration(milliseconds: 100), () {});
  bool isPlaying = false;
  List curMaskData = frame_shape;
